...
class NeuralNetwork:
	def __init__(self, inputs, outputs, nTrainingExamples, stepSize =  0.1, nLayers = 3, regularizationParameter = 5):
		self.Xs = inputs
		self.Ys = outputs
		...
		self.theta = np.ones(self.t * self.t * self.l).reshape(self.l ,self.t, self.t) / 10
	...
	def SGD(self):
		...
		for i in range(10):
			...
			#A = np.array([1 for _t in range(self.t)] for _l in range(self.l - 1)])
			...
			for _l in range(1, self.l):
				for _to in range(self.t): # L
					for _from in range(self.t): # LAYER ANTERIOR (L - 1)
						...
						#D[0][0][0] = (1 / self.m) * delta[_l][_to][_from]
						if _to != 0 :
